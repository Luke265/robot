...
require('iohook/package.json');
const EventEmitter = require('events');
const iohookDir = path.dirname(require.resolve('iohook/package.json'));

const runtime = process.versions['electron'] ? 'electron' : 'node';
const essential = runtime + '-v' + process.versions.modules + '-' + process.platform + '-' + process.arch;
const modulePath = path.join(iohookDir, 'builds', essential, 'build', 'Release', 'iohook.node');
